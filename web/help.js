...
const helpTopics = [
  ...
  ['sources','Fuentes, biblioteca y fichas',[
    ...
    'Cambiar la selección abre un hilo nuevo en el próximo envío para retirar el contexto anterior. La idea inicial y las decisiones del proyecto también se envían. La capacidad depende del modelo. El aviso configurable muestra la última medición de contexto de Codex, cuando está disponible; las fuentes no se recortan en silencio.',
    'Importar copias admite Markdown/TXT UTF-8, EPUB sin DRM y DOCX, hasta 15 MB por archivo. Convierte texto en unidades de hasta 1 MB, sin imágenes ni diseño original. Las copias quedan sin seleccionar para IA; revisá su contenido antes de enviarlo.',
  ...
  ['translation','Traducción literaria: intención y revisión humana',[
    ...
    'Podés trabajar con un capítulo completo, más las fuentes de apoyo seleccionadas. Al aprobarlo, la app prepara el siguiente manuscrito sin traducción según Plan y avance. También podés marcar un único manuscrito para elegir otra unidad; conserva idiomas, intención y glosario. Con varios manuscritos marcados, elegí el original en Preparar encargo. La capacidad de una respuesta depende del modelo.',
    ...
    'La carpeta importada queda inicialmente vinculada para guardar también cada traducción aprobada; podés cambiarla o desactivarla en Traducción. Nunca se sobrescriben archivos existentes. La exportación compilada usa la última copia revisada de cada unidad para el idioma actual; Markdown/DOCX/PDF son ediciones básicas de lectura. La IA puede omitir matices: la lectura humana sigue siendo necesaria.'
  ]],
