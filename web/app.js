...
function workingTime(start, activity) {
  const seconds = Math.max(0, Math.floor(Date.now()/1000-start));
  if (activity) {
    const idle = Math.max(0, Math.floor(Date.now()/1000-activity));
    return `Sin actividad hace ${Math.floor(idle/60)}:${String(idle%60).padStart(2,'0')} · podés esperar o presionar Escape para interrumpir`;
  }
...
setInterval(() => {
  for (const node of document.querySelectorAll('[data-working-since]'))
    node.textContent = workingTime(Number(node.dataset.workingSince), Number(node.dataset.lastActivity));
}, 1000);
function runTimestamp(run) {
  if (['connecting','running'].includes(run.status) && Number.isFinite(run.date))
    return `<span class="run-time" data-working-since="${run.date}" data-last-activity="${run.waiting ? run.last_activity : 0}">${workingTime(run.date, run.waiting ? run.last_activity : 0)}</span>`;
