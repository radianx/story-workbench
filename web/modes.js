...
function applyPurpose(){
  ...
  $('book-progress').hidden=rpg;document.querySelector('.skill-check').hidden=rpg;$('export').textContent=rpg?'Exportar dossier ↓':'Exportar proyecto ↓';
  $('purpose-banner').hidden=purpose==='novel';
  $('purpose-banner').innerHTML=translation?'<strong>Traducción con criterio del autor</strong><p>Primero el encargo; después los matices, el borrador y tu revisión.</p><button class="secondary" data-translation-setup>Preparar encargo</button><button class="context-link" data-help="translation">Cómo funciona</button>':rpg?'<strong>Mundo para rol · modo extra</strong><p>Prepará situaciones abiertas, PNJ, lugares y reglas del mundo para tu mesa.</p><button class="secondary" data-rpg-starter>Preparar un mundo inicial</button><button class="context-link" data-help="rpg">Ayuda de rol</button>':'';
  if(purpose!=='novel'){$('assistant-title').textContent=translation?'Conservemos la intención':'Construyamos tu mundo';$('assistant-eyebrow').textContent=purposeNames[purpose].toUpperCase();}
  ...
  document.querySelector('#mode option[value=draft]').textContent=rpg?'Preparar material de rol':'Redactar borrador';
  if(translation&&$('mode').value==='draft')$('mode').value='translate';
  setTaskMode();
}
$('purpose').onchange=action(async()=>{
  if(dirty){$('purpose').value=state.purpose;throw new Error('Guardá el documento antes de cambiar el objetivo.');}
  const project=state.id;
  state=await api('/api/project/purpose',{project,purpose:$('purpose').value});
  applyWorkflow();savePromptDraft();renderDocuments();renderAssistant();
  if(state.workflow==='guided'&&!hasPurposeInterview())await beginInterview();
});
function prepareTranslation(){
  $('mode').value='translate';setTaskMode();showPanel('conversation');
  $('prompt').value='Continuemos la traducción de la unidad del encargo. Consultá un matiz relevante si falta mi criterio; conservá la intención y las decisiones aprobadas.';
  savePromptDraft();$('prompt').focus();
}
function translationDraft(run,kind,fallback=''){
  return sessionStorage.getItem(`sw-translation-${state.id}-${run}-${kind}`)??fallback;
}
function translationHTML(run){
  if(run.status!=='completed'||!run.translation_result)return '';
  const result=run.translation_result,id=run.id,context=run.translation_context;
  const label=`${context.config.source_language} → ${context.config.target_language}`;
  if(result.question)return `<section class="translation-card"><strong>${escapeHTML(label)} · Matiz para decidir</strong><blockquote>${escapeHTML(result.quote)}</blockquote><p>${escapeHTML(result.question)}</p>${run.translation_answer?`<p class="criterion-saved">Tu criterio: ${escapeHTML(run.translation_answer)}</p><button class="secondary" data-translation-continue>Continuar con IA</button>`:`<div class="nuance-options">${result.options.map((option,i)=>`<button class="secondary" data-nuance="${id}" data-option="${i}"><strong>${escapeHTML(option.wording)}</strong><span>${escapeHTML(option.effect)}</span></button>`).join('')}</div><label>Tu criterio (podés escribir otra alternativa)<textarea data-criterion="${id}" maxlength="4000" rows="3">${escapeHTML(translationDraft(id,'criterion'))}</textarea></label><button class="primary" data-translation-answer="${id}">Registrar mi criterio</button><p>Registrar guarda tu decisión. Continuar con IA inicia otro turno con ella.</p>`}</section>`;
  const saved=state.documents.find(d=>d.source_run===id&&d.translation);
...
function rememberTranslationFocus(){
  const el=document.activeElement,key=el?.dataset.translationText?'translation-text':el?.dataset.criterion?'criterion':null;
  if(!key)return ()=>{};
  const id=el.getAttribute('data-'+key),start=el.selectionStart,end=el.selectionEnd;
  return ()=>{const target=document.querySelector(`[data-${key}="${id}"]`);if(target){target.focus({preventScroll:true});target.setSelectionRange(start,end);}};
}
$('runs').addEventListener('input',event=>{
  const el=event.target,id=el.dataset.criterion||el.dataset.translationText;if(!id)return;
  try{sessionStorage.setItem(`sw-translation-${state.id}-${id}-${el.dataset.criterion?'criterion':'text'}`,el.value);}catch{notice('No se pudo conservar esta revisión en la ventana. Copiala antes de salir.',true);}
});
$('runs').addEventListener('click',action(async event=>{
  const button=event.target.closest('button');if(!button)return;const project=state.id;
  if(button.dataset.nuance){
    const run=state.runs.find(r=>r.id===button.dataset.nuance),option=run.translation_result.options[Number(button.dataset.option)];
    const input=document.querySelector(`[data-criterion="${run.id}"]`);input.value=option.wording+' — '+option.effect;input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();
  }
  if(button.dataset.translationAnswer){
    const run=button.dataset.translationAnswer,answer=document.querySelector(`[data-criterion="${run}"]`).value;
    const incoming=await api('/api/translation/answer',{project,run,answer});if(state.id!==project)return;
    state=incoming;lastRuns='';renderAssistant();notice('Criterio registrado. Continuá cuando quieras.');
  }
  if(button.hasAttribute('data-translation-continue')){prepareTranslation();$('send').click();}
  if(button.dataset.translationAccept){
    if(dirty)throw new Error('Guardá el documento abierto antes de aprobar otra copia.');
    const run=button.dataset.translationAccept,text=document.querySelector(`[data-translation-text="${run}"]`).value;
    await api('/api/translation/accept',{project,run,text});if(state.id!==project)return;
    const incoming=await api(`/api/projects/${project}`);if(state.id!==project)return;state=incoming;renderDocuments();lastRuns='';renderAssistant();notice('Traducción aprobada como copia independiente. El original se conserva.');
  }
  if(button.dataset.openTranslation){showMaterial();openDocument(button.dataset.openTranslation);}
}));
function renderTranslationDocuments(){
  const docs=state.documents.filter(d=>d.translation),locale=state.translation_config?.target_language;
  $('translation-status').textContent=`${docs.filter(d=>d.translation_status==='reviewed'&&d.translation.target_language===locale).length} copias revisadas para ${locale||'el idioma por definir'}. Cada unidad requiere tu aprobación.`;
  $('translation-documents').innerHTML=docs.map(doc=>{
    const original=state.documents.find(d=>d.id===doc.translation.source);
...
function openTranslation(){
  const config=state.translation_config||{};$('translation-form').dataset.dirty='false';
...
function checkTranslationEdits(){if($('translation-form').dataset.dirty==='true')throw new Error('Guardá primero los cambios del encargo.');}
function closeTranslation(event){if($('translation-form').dataset.dirty==='true'&&!confirm('¿Cerrar sin guardar los cambios del encargo?')){event?.preventDefault();return;}$('translation-dialog').close();}
$('translation-close').onclick=closeTranslation;$('translation-dialog').oncancel=closeTranslation;
$('translation-form').onsubmit=action(async event=>{
  event.preventDefault();if(dirty)throw new Error('Guardá primero los cambios del original.');
  const project=state.id,config={source:$('translation-source').value,source_language:$('translation-from').value.trim(),target_language:$('translation-to').value.trim(),intent:$('translation-intent').value,glossary:$('translation-glossary').value};
...
$('translation-documents').onclick=action(async event=>{
  ...
  if(button.dataset.translationReview){if(dirty)throw new Error('Guardá primero el texto del editor.');state=await api('/api/translation/review',{project:state.id,document:button.dataset.translationReview,hash:button.dataset.hash,source_hash:button.dataset.sourceHash});renderTranslationDocuments();renderDocuments();renderAssistant();notice('Revisión humana registrada para ambas versiones.');}
});
