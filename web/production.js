...
function sizeBook() {
  const sizing=$('book-sizing').value, w=Number($('book-width').value), h=Number($('book-height').value);
  $('book-pages').readOnly=sizing!=='pages';$('book-pages').required=sizing!=='manual';
  $('book-thickness').readOnly=sizing!=='manual';$('book-paper').disabled=sizing==='manual';
  let words=0;
  if(sizing==='estimate'){
    ...
    const perPage=300*((w-30)*(h-30))/((152.4-30)*(228.6-30));
    $('book-pages').value=w>=80&&h>=100?Math.max(24,Math.ceil(words/perPage/2)*2):24;
  }
  const pages=Number($('book-pages').value);
  $('book-pages').setCustomValidity(sizing!=='manual'&&(!Number.isInteger(pages)||pages<1||pages>1500)?'Usá entre 1 y 1500 páginas para esta maqueta.':'');
  if(sizing!=='manual')$('book-thickness').value=(pages*($('book-paper').value==='cream'?.0635:.0572)).toFixed(2);
  $('book-estimate').textContent=sizing==='estimate'?`${words.toLocaleString('es')} palabras guardadas · ${pages} páginas estimadas. Base: 300 palabras por página a 6 × 9, márgenes de 15 mm y mínimo visual de 24 páginas. No incluye portadillas ni imágenes.`:sizing==='pages'?'Cantidad total de páginas de tu edición (cada cara cuenta), no cantidad de hojas.':'Se conserva el grosor indicado por tu imprenta.';
}
function renderBook() {
  sizeBook();
  const w = Number($('book-width').value), h = Number($('book-height').value), d = Number($('book-thickness').value);
  if (w >= 80 && w <= 300 && h >= 100 && h <= 400 && d >= .01 && d <= 100) {
...
$('book-open').onclick = () => {
  ...
  book = {width:152.4, height:228.6, spine:20, sizing:state.production?'manual':'estimate', pages:24, paper:'white', title:state.title, author:'', front:'', back:'', spineImage:'', ...state.production};
...
$('book-default-size').onclick=()=>{$('book-width').value=152.4;$('book-height').value=228.6;$('book-form').oninput();};
...
$('book-form').onsubmit = action(async event => {
  ...
  const production = {...book, sizing:$('book-sizing').value, pages:Number($('book-pages').value), paper:$('book-paper').value, width:Number($('book-width').value), height:Number($('book-height').value), spine:Number($('book-thickness').value), title:$('book-name').value, author:$('book-author').value};
